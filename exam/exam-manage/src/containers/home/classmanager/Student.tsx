...
class Student extends Component<IProps, IState> {
  ...
  search = () => {
      let arr:any =[]
      if(!this.state.studentUser && !this.state.className && !this.state.classRoomname){
        return;
      }
      if (this.state.studentUser) {// 根据姓名搜索
         arr=[...arr,...this.state.list.filter(item => item.student_name.includes(this.state.studentUser))]
      }
  ...
  getList = async () => {
    ...
    if (result.data.code === 1) {
      ...
      let list = result.data.data.map((item: any, index: any) => {
        return { ...item,student_pwd:(Base64.stringify(Utf8.parse(`${item.student_pwd}`))), key: index }
      })
      this.setState({
        list: list
      })
